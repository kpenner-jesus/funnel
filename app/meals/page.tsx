...
export default function MealsPage() {
  ...
  const getMealCost = (mealKey: string) => {
    ...
    const kids  = children > 0 && childrenAvgAge > 0
      ? (mealKey === "nightsnack" ? SITE_CONFIG.meals.childNightSnackRate : SITE_CONFIG.meals.childMealRatePerYear)
        * childrenAvgAge * children
      : 0;
    return adult + kids;
  };
      ...
      <div className="tf-body" style={{ justifyContent: "flex-start", paddingTop: "3rem" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "0.875rem", width: "100%" }}>
          {days.map((day, di) => {
            ...
            return (
              <div key={day.date}
                className={`tf-invoice-section tf-animate tf-animate-delay-${Math.min(di + 2, 8)}`}>
                {/* Day header */}
                <div style={{
